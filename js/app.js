...
function renderPage(page) {
  switch(page) {
    ...
    case 'tabela-coins':   renderTabelaCoins();  break;
  }
}
...
// PONTOS
// ============================================================
// Mapa de valores automáticos por motivo
const MOTIVOS_VALOR = {
  'Excelente Comportamento': 20,
  'Realizou todas as atividades da semana': 10,
  'Frequência Escolar Impecável': 10,
  'Fez todos os Para Casa': 15,
  'Ajudou um Colega ou a Escola': 10,
  'Participou Ativamente da aula': 10,
  'Demonstra Interesse em Aprender': 5,
  'Praticou Bullying': 20,
  'Faltas em excesso': 10,
  'Não fez as Atividades': 5,
  'Desrespeitou as Regras': 15,
  'Agrediu colega': 0,
};
...
function abrirModalPontos(alunoId, acao = 'ganho') {
  ...
  // Preencher valor automaticamente ao mudar motivo
  document.getElementById('pts-motivo').onchange = function() {
    const motivo = this.value;
    const val = MOTIVOS_VALOR[motivo];
    if (val !== undefined) {
      if (motivo === 'Agrediu colega') {
        document.getElementById('pts-valor').value = a.pontos || 0;
        document.getElementById('pts-valor').disabled = true;
      } else {
        document.getElementById('pts-valor').value = val;
        document.getElementById('pts-valor').disabled = false;
      }
...
async function salvarPontos() {
  ...
  // Caso especial: Agrediu colega → zera todos os pontos
  if (motivo === 'Agrediu colega') {
    const pontosAtuais = Number(a.pontos) || 0;
    if (pontosAtuais === 0) {
      showToast('Aluno já está com 0 pontos', 'warning');
      return;
    }
    if (!confirm(`⚠️ Isso vai ZERAR todos os ${pontosAtuais} pontos de ${a.nome}. Confirmar?`)) return;
    try {
      await api(`${API.alunos}/${a.id}`, 'PATCH', { pontos: 0 });
      await api(API.historico, 'POST', {
        aluno_id: a.id, pontos: -pontosAtuais, motivo: 'Agrediu colega',
        tipo: 'zerado', data: Date.now()
      });
      showToast(`⚠️ Pontos de ${a.nome} zerados por agressão!`, 'warning');
      closeModal('modal-pontos');
      await loadAll(); renderAlunos(); renderDashboard();
    } catch(err) { showToast('Erro ao zerar pontos', 'error'); }
    return;
  }
  ...
  try {
    ...
    await api(API.historico, 'POST', { aluno_id: a.id, pontos: ptsAcao === 'ganho' ? pts : -pts, motivo, tipo: ptsAcao, data: Date.now() });
...
// TABELA COINS
// ============================================================
function renderTabelaCoins() {
  // Página estática no HTML — futuramente pode carregar da API coin_motivos
}

// ============================================================
// MÚLTIPLOS PROFESSORES POR TURMA
// ============================================================
function getProfessoresDaTurma(turmaId) {
  if (state.professorTurma && state.professorTurma.length > 0) {
    const vinculos = state.professorTurma.filter(pt => String(pt.turma_id) === String(turmaId));
    return vinculos.map(pt => state.professoras.find(p => String(p.id) === String(pt.professor_id))).filter(Boolean);
  }
  const t = state.turmas.find(x => String(x.id) === String(turmaId));
  const p = t ? state.professoras.find(p => String(p.id) === String(t.professora_id)) : null;
  return p ? [p] : [];
}
